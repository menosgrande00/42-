...
/*   Updated: 2025/08/11 01:42:10 by omerfarukon      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*replace_tmp_envp(char *tmp, int *dollar_idx, char *value, int key_len)
{
    ...
    prefix_len = *dollar_idx;                 // '$' öncesi
    suffix_start = *dollar_idx + 1 + key_len; // $KEY sonrası ilk index
    new_len = prefix_len + ft_strlen(value) + ft_strlen(&tmp[suffix_start]);
...
static int	money_money_process(t_minishell *minishell, char **tmp, int *j)
{
    ...
    free(*tmp);
    *tmp = new_tmp;
    // Sonraki tarama için eklenen değerin sonuna yerleştir
    *j = dollar + (int)val_len - 1;
    return (0);
}
// ...existing code...
