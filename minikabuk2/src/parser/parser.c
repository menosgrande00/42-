...
/*   Updated: 2025/08/11 01:59:46 by omerfarukon      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	remove_empty_unquoted_tokens_list(t_token_list **list)
{
	...
	while (cur)
	{
		next = cur->next;
		if (cur->token
			&& cur->token->type == TOKEN_WORD
			&& cur->token->quotes == 0
			&& (!cur->token->value || cur->token->value[0] == '\0'))
		{
			if (cur->prev)
...
void	parse_tokens(t_minishell *minishell, t_token_list *tmp)
{
	...
	// Drop empty unquoted words created by env expansion of unset vars
