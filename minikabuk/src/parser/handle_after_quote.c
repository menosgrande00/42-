...
static char *handle_single_quote_segment(const char *input, int *pos_ptr, char *result)
{
        int qs;
        int qe;
        char *segment;
        ...
        if (input[qe] != '\'')
                return (result);
        segment = ft_substr(input, qs + 1, qe - qs - 1);
        if (!segment)
                return (result);
        k = 0;
        while (segment[k])
...
char    *collect_after_quot(const char *input, int start_idx, int *out_end_pos)
{
    ...
    while (input[pos] && input[pos] != ' ')
    {
            if (input[pos] == '"')
                    ...
                    result = handle_single_quote_segment(input, &pos, result);
            else
